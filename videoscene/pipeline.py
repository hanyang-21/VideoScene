...
from .LCMScheduler import LCMScheduler
from .utils import process_video, prepare_rotary_positional_embeddings
...
class VideoScenePipeline(DiffusionPipeline): 
    ...
    def __init__(
        ...
        scheduler: Union[CogVideoXDDIMScheduler, CogVideoXDPMScheduler, LCMScheduler],
    ):
        super().__init__()
    ...
    def prepare_latents(
        ...
        # latent_padding = torch.zeros(shape, device=device, dtype=dtype)
        # image_latents = torch.cat([latent_padding], dim=1)
    ...
    def __call__(
        ...
        moisy_timestep = 500
        moisy_timesteps = torch.randint(
            moisy_timestep-1, moisy_timestep, (1,), device=latents.device
        )
        moisy_timesteps = moisy_timesteps.long()
        ...
        latents = self.scheduler.add_noise(latents, noise, moisy_timesteps)
    ...
    @torch.no_grad()
    def video_gen(self, video_path, input_images, prompt_embeds, noise_timestep):
        noise_t=noise_timestep
        _, h, w, _ = input_images.shape
        images = torch.from_numpy(input_images).permute(0, 3, 1, 2).float().cuda() # [V, H, W, C] -> [V, C, H, W]
        first_image = images[0].clone().detach().unsqueeze(0)
        last_image = images[1].clone().detach().unsqueeze(0) # [C, H, W] -> [F, C, H, W]

        video = process_video(video_path)
        video = 2 * (video/255.0) - 1.0
        video = video.unsqueeze(0).permute(0, 2, 1, 3, 4)
        video = video.to('cuda', dtype=self.dtype)
        video_latents = self.vae.encode(video).latent_dist.sample() * self.vae.config.scaling_factor
        first_image = self.video_processor.preprocess(first_image, height=480, width=720).to(
            'cuda', dtype=self.dtype
        )
        last_image = self.video_processor.preprocess(last_image, height=480, width=720).to(
            'cuda', dtype=self.dtype
        )
        first_image = first_image.unsqueeze(2)
        last_image = last_image.unsqueeze(2)
        first_image = first_image.to(dtype=self.dtype)
        last_image = last_image.to(dtype=self.dtype)
        ...
        first_image_latents = self.vae.encode(first_noisy_image).latent_dist.sample() * self.vae.config.scaling_factor
        
        last_noisy_image = last_image + torch.randn_like(last_image) * image_noise_sigma[:, None, None, None, None]
        last_image_latents = self.vae.encode(last_noisy_image).latent_dist.sample() * self.vae.config.scaling_factor
        ...
        timesteps = torch.randint(
            noise_t-1, noise_t, (1,), device=video_latents.device
        )
        ...
        prompt_embeds = prompt_embeds.to(device=video_latents.device, dtype=self.dtype)
        latents = noisy_video_latents

        t = timesteps[0]
        ts = t.expand(latents.shape[0])
        noisy_model_input = torch.cat([latents, image_latents], dim=2).to(dtype=self.dtype)   
        ...
        output_video = self.scheduler.get_velocity(model_output, latents, ts)
        video_s = output_video.permute(0, 2, 1, 3, 4)
        video_s = 1 / self.vae.config.scaling_factor * video_s
        video_s = self.vae.decode(video_s).sample
        output_video = self.video_processor.postprocess_video(video=video_s, output_type="pil")

        output = [frame.resize((w,h)) for frame in output_video[0]]

        output_path = video_path.split('_tmp')[0] + '.mp4'
