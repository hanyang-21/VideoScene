...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        # video generation
        pretrain_ckpt: str = test_cfg.pretrain
        video_ckpt: str = test_cfg.video
        self.video_model = VideoScenePipeline.from_pretrained(
            pretrain_ckpt,
            torch_dtype=torch.bfloat16
        ).to('cuda')

        # load video checkpoint
        state_dict = safetensors.torch.load_file(video_ckpt)
        self.video_model.transformer.load_state_dict(state_dict, strict=False)
        
        self.prompt_embeds = torch.load("checkpoints/prompt_embeds.pt", map_location='cpu')
    ...
    def test_step(self, batch, batch_idx):
        ...
        images = batch["context"]["image"].squeeze(0).permute(0, 2, 3, 1)
        images = images.cpu().numpy()
        video_result_path = self.video_model.video_gen(f'{path}/video/{scene}_tmp.mp4', images, self.prompt_embeds, noise_timestep=550)

        os.system(f"rm {path}/video/{scene}_tmp.mp4")
