...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        self.chunks = []
        content_list = []
        self.index_list = self.view_sampler.index

        for root in cfg.roots:
            with open(f'{root}/{self.data_stage}/index.json', 'r') as f:
                all_index_list = json.load(f)
            for key in self.index_list.keys():
                if key in all_index_list:
                    self.chunks.append(f"{root}/{self.data_stage}/{all_index_list[key]}")

        # for root in cfg.roots:
        #     root = root / self.data_stage
        #     root_chunks = sorted(
        #         [path for path in root.iterdir() if path.name in ["000753.torch","003215.torch","003565.torch"]]  #path.suffix == ".torch"]
        #     )
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for run_idx in range(int(times_per_scene * len(chunk))):
                ...
                context_image_invalid = context_images.shape[1:] != (3, 480, 720)
                target_image_invalid = target_images.shape[1:] != (3, 480, 720)
    ...
    def __len__(self) -> int:
        return (len(self.index_list.keys()))
